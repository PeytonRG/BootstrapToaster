...
class Toast {
  ...
  static enableQueue(enabled = true) {
    this.queueEnabled = enabled; // Empty the queue once it's disabled.
  ...
  static create(toastOptions) {
    ...
    this.setStatus(toastEl, toastOptions.status); // Add toast to the queue if it would exceed maxToastCount
  ...
  static render(toastInfo) {
    ...
    bsToast.show();
    this.currentToastCount++; // When the toast hides, remove it from the DOM
...
export { TOAST_PLACEMENT, TOAST_STATUS, TOAST_THEME, TOAST_TIMERS, Toast };
